...
public class teacher extends JFrame implements ActionListener{
	...
	String st_id="";
	String st_name="";
	public teacher(String clid,String name2,String id2) {
		// TODO Auto-generated constructor stub
		newclid=clid;
		st_id=id2;
		st_name=name2;
		this.setTitle("教师信息管理--欢迎查阅");
		jmb = new JMenuBar();
		jm = new JMenu("主页");
		...
		if(conn!=null){
			String tel="",type="",address="",gender="",name="",id="";
			try {
				String sql ="select * from teacher where th_position='" + clid+"'";
				Statement s = conn.createStatement();
				ResultSet rs = s.executeQuery(sql);
				while (rs.next()) {
					 tel = rs.getString("th_tel");
					 id =rs.getString("th_id");
					 address =rs.getString("th_address");
					 name=rs.getString("th_name");
					 gender=rs.getString("th_gender");
					 type=rs.getString("th_type");
						}
				jl7.setText(gender);
				jl8.setText(type);
				jl11.setText(tel);
